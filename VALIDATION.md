...
Validaciones ejecutadas para la versión `0.4.4`:
...
- Exportación Excel: elegibilidad, generación XLSX, metadatos, nombre seguro y escritura literal de strings validada.
- Pruebas ejecutadas: **42 aprobadas y 4 omitidas** en la suite completa disponible.
...
4. La prueba unitaria de extracción de relaciones físicas del plan requiere `psycopg` para importar el adaptador PostgreSQL.
...
10. Ejecutar una consulta tabular, preparar el Excel y verificar las hojas `Resultados` y `Metadatos`.
11. Confirmar que una consulta truncada no ofrece descarga cuando `EXCEL_EXPORT_ALLOW_TRUNCATED=false`.
...
## Exportación Excel (incorporada en 0.4.3)

- Implementada como `ExcelExportTool`, no como agente LLM.
- Solo exporta runs completados con resultados tabulares no vacíos.
- Bloquea resultados truncados por defecto.
- Genera hojas `Resultados` y `Metadatos`.
- Usa XlsxWriter en memoria y no crea archivos temporales en el contenedor.
- Deshabilita la conversión automática de strings a fórmulas o enlaces para reducir riesgos de spreadsheet injection.
- El endpoint valida ownership del run y registra `excel_exported` en auditoría.
- Streamlit muestra la opción de forma condicional y mantiene el archivo preparado en memoria de sesión.


## Validación visible y contratos 0.4.4

- `RunResponse` expone `security_validation` y `cost_validation`.
- Streamlit renderiza un panel persistente con resultados, límites y plan `EXPLAIN`.
- El streaming muestra resumen de seguridad y costo en tiempo real.
- `SecurityValidation` publica reglas aplicadas y límite efectivo.
- `CostValidation` publica límites, fuentes semánticas, relaciones físicas, plan y timeout.
- README documenta input, output y responsabilidad de cada agente y tool sin duplicar la antigua tabla resumida.
